...
- (void)startRequest {
    ...
    [self resetData];
    [self accessRequestSerializer];//  请求序列化，可以设置请求信息
    [self accessResponseSerializer];//响应序列化，可以设置相应信息，比如以json返回
    [self accessHeaderField];//设置http 头
    [self accessTimeoutInterval];//设置超时
    
    if ([self.method isKindOfClass:[GetMethod class]]) {
        
        [self accessGetRequest];//开始get请求
        
    } else if ([self.method isKindOfClass:[PostMethod class]]) {
        
        [self accessPostRequest];//开始post请求
    }
